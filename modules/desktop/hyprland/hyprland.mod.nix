  ...
  flake.homeModules.hyprland =
    ...
    in
    {
      ...
      wayland.windowManager.hyprland = {
        ...
        #
        # All plugin-coupled config lives in the guarded blocks below,
        # not in settings: hl.plugin.load only registers a path — the
        # .so loads after the config's first execution, so on a fresh
        ...
        settings = {
          ...
          config = {
            ...
            # gloview and hyprbars plugin config lives in the guarded
            # extraConfig blocks above — from here it would error on the
            # first config pass, before plugins load.
